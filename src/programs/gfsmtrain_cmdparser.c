...
{
  printf("gfsmtrain (%s %s) by Bryan Jurish <moocow.bovine@gmail.com>\n", PACKAGE, VERSION);
}
...
cmdline_parser_print_help (void)
{
  ...
  printf("\n");
  printf("Purpose:\n");
  printf("  EXPERIMENTAL: count successful paths for training string pairs in a transducer\n");
  printf("\n");
  
  printf("Usage: %s [OPTIONS]... PAIR_FILE(s)...\n", "gfsmtrain");
  
  printf("\n");
  printf(" Arguments:\n");
  printf("   PAIR_FILE(s)...  Input string-pair file(s)\n");
  ...
  printf("   -fFSTFILE  --fst=FSTFILE         Transducer to apply (required).\n");
  ...
  switch (oshort)
    {
      case 'h':	 /* Print help and exit. */
        ...
        case 'O':	 /* Count permutations in arc-order as multiple paths. */
          if (args_info->ordered_given) {
            fprintf(stderr, "%s: `--ordered' (`-O') option given more than once\n", PROGRAM);
          }
          args_info->ordered_given++;
         if (args_info->ordered_given <= 1)
           args_info->ordered_flag = !(args_info->ordered_flag);
          break;
        
        case 'P':	 /* Distribute pair-mass over multiple paths. */
          if (args_info->distribute_by_path_given) {
            fprintf(stderr, "%s: `--distribute-by-path' (`-P') option given more than once\n", PROGRAM);
          }
          args_info->distribute_by_path_given++;
         if (args_info->distribute_by_path_given <= 1)
           args_info->distribute_by_path_flag = !(args_info->distribute_by_path_flag);
          break;
        
        case 'A':	 /* Distribute path-mass over arcs. */
          if (args_info->distribute_by_arc_given) {
            fprintf(stderr, "%s: `--distribute-by-arc' (`-A') option given more than once\n", PROGRAM);
          }
          args_info->distribute_by_arc_given++;
         if (args_info->distribute_by_arc_given <= 1)
           args_info->distribute_by_arc_flag = !(args_info->distribute_by_arc_flag);
          break;
        
        case 'f':	 /* Transducer to apply (required). */
        ...
        /* Print help and exit. */
          ...
          else if (strcmp(olong, "ordered") == 0) {
            if (args_info->ordered_given) {
              fprintf(stderr, "%s: `--ordered' (`-O') option given more than once\n", PROGRAM);
            }
            args_info->ordered_given++;
          ...
          else if (strcmp(olong, "distribute-by-path") == 0) {
            if (args_info->distribute_by_path_given) {
              fprintf(stderr, "%s: `--distribute-by-path' (`-P') option given more than once\n", PROGRAM);
            }
            args_info->distribute_by_path_given++;
          ...
          else if (strcmp(olong, "distribute-by-arc") == 0) {
            if (args_info->distribute_by_arc_given) {
              fprintf(stderr, "%s: `--distribute-by-arc' (`-A') option given more than once\n", PROGRAM);
            }
            args_info->distribute_by_arc_given++;
