...
{
  printf("gfsmarcsort (%s %s) by Bryan Jurish <moocow.bovine@gmail.com>\n", PACKAGE, VERSION);
}
...
cmdline_parser_print_help (void)
{
  ...
  printf("   -i                         Alias for '--lower'\n");
  printf("   -o                         Alias for '--upper'\n");
  printf("   -C       --cost            Alias for '--weight'\n");
  printf("   -mMODE   --mode=MODE       Sort by explicit mode string (overrides -l, -u, etc.)\n");
  printf("   -U       --unique          After sorting, collect weights on otherwise identical arcs\n");
  ...
  switch (oshort)
    {
      case 'h':	 /* Print help and exit. */
        ...
        case 'U':	 /* After sorting, collect weights on otherwise identical arcs */
          if (args_info->unique_given) {
            fprintf(stderr, "%s: `--unique' (`-U') option given more than once\n", PROGRAM);
          }
          args_info->unique_given++;
         if (args_info->unique_given <= 1)
           args_info->unique_flag = !(args_info->unique_flag);
          break;
