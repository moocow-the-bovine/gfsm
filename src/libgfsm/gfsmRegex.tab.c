...
#line 9 "gfsmRegex.tab.y" /* yacc.c:339  */
...
do                                                              \
  if (yychar == YYEMPTY)                                        \
    ...
    {                                                           \
      yyerror (reparser, YY_("syntax error: cannot back up")); \
      YYERROR;                                                  \
    }                                                           \
while (0)
...
static void
yy_symbol_value_print (FILE *yyoutput, int yytype, YYSTYPE const * const yyvaluep, gfsmRegexCompiler* reparser)
{
  FILE *yyo = yyoutput;
  YYUSE (yyo);
  YYUSE (reparser);
...
static void
yy_symbol_print (FILE *yyoutput, int yytype, YYSTYPE const * const yyvaluep, gfsmRegexCompiler* reparser)
{
  YYFPRINTF (yyoutput, "%s %s (",
             yytype < YYNTOKENS ? "token" : "nterm", yytname[yytype]);

  yy_symbol_value_print (yyoutput, yytype, yyvaluep, reparser);
  YYFPRINTF (yyoutput, ")");
}
...
static void
yy_reduce_print (yytype_int16 *yyssp, YYSTYPE *yyvsp, int yyrule, gfsmRegexCompiler* reparser)
{
...
do {                                    \
  if (yydebug)                          \
    yy_reduce_print (yyssp, yyvsp, Rule, reparser); \
} while (0)
...
static void
yydestruct (const char *yymsg, int yytype, YYSTYPE *yyvaluep, gfsmRegexCompiler* reparser)
{
  YYUSE (yyvaluep);
  YYUSE (reparser);
  if (!yymsg)
...
int
yyparse (gfsmRegexCompiler* reparser)
{
  ...
  if (!yyerrstatus)
    {
      ++yynerrs;
#if ! YYERROR_VERBOSE
      yyerror (reparser, YY_("syntax error"));
#else
...
`-------------------------------------------------*/
yyexhaustedlab:
  yyerror (reparser, YY_("memory exhausted"));
  yyresult = 2;
  /* Fall through.  */
#endif
